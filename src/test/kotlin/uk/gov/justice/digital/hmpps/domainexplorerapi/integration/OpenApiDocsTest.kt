  ...
  fun `the swagger json don't contain any duplicate methods`() {
    ...
    webTestClient.get()
      ...
      .expectBody().jsonPath("*..operationId").value<List<String>> { list ->
        // No operationId should have an autogenerated numeric suffix like _1, _2
        val suffixed = list.filter { it.matches(Regex(".*_\\d+$")) }
        assertThat(suffixed).withFailMessage("Found operationIds with numeric suffixes: $suffixed").isEmpty()

        // Also ensure there are no duplicate operationIds
        val duplicates = list.groupingBy { it }.eachCount().filter { it.value > 1 }.keys
        assertThat(duplicates).withFailMessage("Found duplicate operationIds: $duplicates").isEmpty()
      }
  }
  ...
  fun `the open api json contains the version number`() {
    webTestClient.get()
      ...
      .expectBody().jsonPath("info.version").value<String> {
        assertThat(it).startsWith("1.0.0")
      }
  }
